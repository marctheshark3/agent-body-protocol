...
## 90-second script (no hardware)

Unplug / walk / pad / thinking stays blue. No Lamp, no pad, no network.

```bash
python3 -m pip install -e '.[test]'
python3 -m unittest tests.test_power tests.test_help_rails tests.test_schema tests.test_map -q

# 1. Unplug — leave the wall brick. Mains stays quiet (power is background).
agent-body power --sim mains --record /tmp/power-mains.json

# 2. Walk — 3S-ish pack sim, healthy 11.1 V, origin=sim.
agent-body power --sim battery --record /tmp/power-battery.json

# 3. Pad — drop on Qi. Slow white breathing. origin=sim. Not a 10th event.
...
2. **Walk** — pack lives in the **base**, later. Low overlay is a slow dim solid, **not** `waiting_for_user` amber, **no look-at-user**.
3. **Pad** — Qi white breathing. Move bench to bench, set it down.
4. **Thinking stays blue** — `[0,80,255]` breathing. Power is parallel telemetry, not a 10th brain event.

Then the honest line: **stock Qi is ~5–15 W.** Idle/trickle, not a continuous servo dance. `completed` on Qi **refuses `happy_wiggle`**. Work sessions still want the pad or USB-C. `HAL_SIMULATE` has no `/power`; `--hal` 404s and the CLI falls back to **labeled** sim.
...
- `protocol/power.schema.json` — required `{v, kind, ts, voltage_v, source, origin}`.
- `mapper/power.py` — validate, simulate, map to **existing** `/led/*` markers only.
- `get_power()` is a **GET**. `dispatch()` stays POST-only and **refuses** `POST /power`.
- `agent-body power --sim SOURCE` / `--record FILE` / `--hal URL`.
...
|---|---|---|---|
| `mains` | 12 V wall rail | ~11–14 V | 5 V is **not** mains-low |
| `battery` | 3S pack | ~9.0–12.6 V | LED-low policy if `< 10.5 V` or `soc_pct < 15` |
| `qi` | 5 V Qi RX | ~4.5–5.5 V | 5.0 V is **normal**, not low |

`soc_from_voltage` is a **3S OCV lookup table** for sim. It is **not a BMS**. No current. No temperature. `low` is an **LED overlay**, not a pack cutoff. A real BMS (Phase 2) owns cutoff, balancing, and thermal.

### Qi wattage (do not oversell)

Stock Qi is about **5–15 W**. Enough to hold idle LEDs and trickle a pack. **Not** enough for continuous servo dance, Follow, or `happy_wiggle` as a primary load. Mapper **refuses the wiggle** when `source=qi`. A work session still wants the pad underneath or USB-C.
...
| `qi` and charging | Slow white breathing |
| charging and docked (USB-C / dock, not the Qi case) | Dim green solid |
| `mains` | No extra LED |
