...
- Hatch hop-then-look joint motion is proven by VirtualBody in `tests/test_skills.py`. Not a camera recapture. No HAL_SIMULATE `wake_up` recapture is locked yet. Do not claim camera hatch.
...
- `docs/demo/abp-room.mp4` on main is still a dashboard take / frozen-pose LED slideshow. Do not claim it is this lock. Named aim on HAL_SIMULATE snaps; that is not tracking and not Luxo hatch.
